...
const regions = [
  ...
  'Камчатский край', 'Кемеровская область', 'Кировская область', 'Костромская область',
  ...
  'Орловская область', 'Пензенская область', 'Пермский край', 'Псковская область',
  ...
  'Ульяновская область', 'Челябинская область', 'Забайкальский край', 'Ярославская область',
  'г. Москва', 'г. Санкт-Петербург', 'Еврейская автономная область', 'Ненецкий автономный округ',
  'Ханты-Мансийский АО - Югра', 'Чукотский автономный округ', 'Ямало-Ненецкий автономный округ', 'Республика Крым',
  'г. Севастополь', 'Запорожская область', 'Донецкая Народная Республика', 'Луганская Народная Республика', 'Херсонская область',
]
...
const Modal = ({ open, type, onClose }: ModalProps) => {
  if (!open) return null
  return (
...
const Header = ({
  ...
  const isActive = (path: string) => currentPath === path || (path === '/news' && currentPath.startsWith('/news'))
...
const HomePage = ({ openModal }: { openModal: (t: string) => void }) => {
  ...
  const cards = [
    ...
    { id: 'tutors', icon: '👨‍🏫', title: 'Репетиторы', description: 'Найди своего учителя для подготовки', color: '#F59E0B', link: '/tutors' },
  ]
...
const AppContent = () => {
  ...
  // Текущий путь для подсветки активной кнопки в шапке
  ...
  return (
    <div className="app">
      ...
      <CookieBanner
        ...
        onMore={() => openModal('Политика конфиденциальности')}
      />
